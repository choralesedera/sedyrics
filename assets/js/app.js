const CONTENT_CACHE="sedyrics-content-v2";
...
function renderMarkers(){
  const host=$("markerList");host.innerHTML='';const cat=(currentSong?.markers||[]).map(m=>({...m,source:'catalog'}));const local=localMarkers().map(m=>({...m,source:'local'}));const all=[...cat,...local].sort((a,b)=>a.time-b.time);
  if(!all.length){host.innerHTML='<span style="color:#78909E;font-size:9px">Tsy mbola misy repère. Afaka manampy eto ambany ianao.</span>';return}
  all.forEach(m=>{const b=document.createElement('button');b.className='marker-chip';b.innerHTML=`${escapeHtml(m.label)} <small>${fmt(Number(m.time))}</small>`;b.onclick=()=>{audio.currentTime=Number(m.time)||0;toast(`Repère: ${m.label}`)};host.appendChild(b)})
}
function addMarker(){if(!currentSong)return;const label=$("markerLabel").value.trim();if(!label){toast('Ampidiro ny anaran\'ny repère');return}const list=localMarkers();list.push({label,time:Math.round(audio.currentTime*10)/10});writeJSON(markerKey(),list);$("markerLabel").value='';renderMarkers();toast('Repère voatahiry')}
